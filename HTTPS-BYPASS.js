...
// ===== SIMULADOR DE NAVEGADORES (bypass.js) =====
...
const generateHeaders = (browser, targetHost) => {
    const versions = {
    ...
    const headersMap = {
        brave: {
            ":method": "GET",
            ":authority": Math.random() < 0.5 ? targetHost + (Math.random() < 0.5 ? "." : "") : "www." + targetHost + (Math.random() < 0.5 ? "." : ""),
            ":scheme": "https",
...
class AdvancedRedirectHandler {
    ...
    getRandomCountryCode() {
        const countryCodes = ['TR', 'US', 'VN', 'JP', 'DE', 'FR', 'GB', 'CN', 'IN', 'BR', 'AU', 'CA', 'RU', 'KR', 'SG'];
...
// ===== INICIALIZAÇÃO DO SIMULADOR DE NAVEGADORES =====
// Só inicializa se o host foi definido corretamente
let browserHeaders = {};
let h2settings = {};
let h2_config = [];
if (host && host.startsWith('https://')) {
    try {
        const targetHost = new URL(host).hostname;
        const browser = getRandomBrowser();
        browserHeaders = generateHeaders(browser, targetHost);
        h2settings = h2Settings(browser);
        h2_config = transformSettings(Object.entries(h2settings));
    } catch (e) {
        // Ignora erro se host não for válido
    }
}
// ===== FIM INICIALIZAÇÃO DO SIMULADOR =====
                ...
                try {
                    ...
                    req.on('response', async (headers) => {
                        ...
                        if (Date.now() - lastLogTime >= 3000) {
                            ...
                            const label = '\x1b[38;2;7;140;255mG\x1b[38;2;21;130;255me\x1b[38;2;35;121;255mn\x1b[38;2;49;112;255mi\x1b[38;2;63;102;255ms\x1b[38;2;77;93;255my\x1b[38;2;91;84;255ms\x1b[0m';
